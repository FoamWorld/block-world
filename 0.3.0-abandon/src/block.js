...
class 半砖 extends Block{ // 0为上面那块，之后顺时针旋转
    ...
    show(x,y){
        if(this.dir==0)draw.drawImage(this.w.imgsource(),x,y,32,16)
        else if(this.dir==1)draw.drawImage(this.w.imgsource(),x+16,y,16,32)
        else if(this.dir==2)draw.drawImage(this.w.imgsource(),x,y+16,32,16)
...
class 结构方块 extends Solid{
    static hasgui=true
    constructor(n="",x=0,y=0,le="",wi=""){super();this.n=n;this.x=x;this.y=y;this.le=le;this.wi=wi;}
    putextra(x,y){
        this.x=x;this.y=y;
    }
    onguiopen(){
...
class 合成台 extends Solid{
    ...
    work(){
        ...
        let mh=Math.floor(it.stack/tr.b)
        if(mh==0){
